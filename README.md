...
`--engine hybrid`（Google翻訳 + LLM自然化）指定時のみ使用します。
...
- `auto_translate.py`
  ...
  - `hybrid` モードではGoogle翻訳後に一部セグメントをLLMで自然化
...
1. テキスト抽出
2. segment化
3. 翻訳エンジンで翻訳（デフォルト: Google / hybrid対応）
4. レイアウト維持でPDF再生成
...
GOOGLE_TIMEOUT=30
HYBRID_MAX_SEGMENTS=40
HYBRID_MIN_CHARS=100
HYBRID_MIN_SCORE=2
HYBRID_WORKERS=2
...
python3 auto_translate.py input.pdf --engine hybrid
# -> input_HYBRID.pdf
```
...
python3 auto_translate.py <source_pdf> [output_pdf] \
  --engine google \
  --google-timeout 30 \
  --hybrid-max-segments 40 \
  --hybrid-min-chars 100 \
  --hybrid-min-score 2 \
  --hybrid-workers 2 \
